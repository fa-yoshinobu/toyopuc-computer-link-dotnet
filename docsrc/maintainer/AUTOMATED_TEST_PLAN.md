...
- Cover:
  ...
  - `examples/Toyopuc.WriteLimitProbe`
  - helper batch files such as `soak_monitor_10gx_core.bat`
...
- For `Toyopuc.DeviceMonitor`, move profile/catalog-dependent selection logic into pure helpers so it can be tested without UI automation.
...
- generated smoke suites in `examples/Toyopuc.SmokeTest`
...
- Keep it separate from `Toyopuc.SmokeTest` so short validation and long-running observation do not share the same execution model.
...
- Run the long-duration monitoring with `examples/Toyopuc.SoakMonitor`.
...
- Relay continuous-length limit characterization is still open.
  ...
  - Current helper artifacts:
    ...
    - `examples/Toyopuc.SmokeTest --probe-counts`
